...
Digitize paper documents, extract formatted text, translate into dozens of languages offline, and synchronize seamlessly across Google Drive and Supabase cloud storage.
...
### ☁️ Dual Cloud Sync & File Explorer
...
* **Supabase Cloud Backend**: Secure user authentication (Email/Password & Google OAuth) with deep-link verification (`docunova://auth`) and document backup in Supabase Storage (`user-documents`).
...
* **Lottie Animations**: Fluid vector animations for dialogs, loading indicators, and email confirmation status.
...
| **Backend as a Service**| Supabase Kotlin SDK (2.4.0) | Auth (GoTrue), Database (PostgREST), Storage (`user-documents`) |
| **Networking** | Retrofit 2.9 + OkHttp + Volley | RESTful API client with logging interceptors and network security |
...
│   │   │   │   ├── RetrofitClient.kt                # Retrofit HTTP client builder
│   │   │   │   ├── SupabaseAuthApi.kt               # Supabase authentication interface
│   │   │   │   ├── User.kt                          # User domain data model
│   │   │   │   ├── MainActivity.kt                  # Bottom navigation shell
│   │   │   │   ├── SplashActivity.kt                # Animated launch & auto-login check
│   │   │   │   ├── LoginActivity.kt                 # Email & Google Sign-In authentication
│   │   │   │   ├── RegisterActivity.kt              # Account registration & profile setup
│   │   │   │   ├── EmailConfirmationActivity.kt     # Deep-link auth callback handler
...
│   │   │   ├── res/
│   │   │   │   ├── layout/                          # 26 XML layouts (Material 3 cards, shimmers)
│   │   │   │   ├── drawable/                        # 67 vector assets, icons & custom gradients
...
Docunova utilizes Supabase for secure profile storage and document bucket archiving:
1. Create a project on [Supabase.com](https://supabase.com).
2. Under **Storage**, create a public or authenticated bucket named `user-documents`.
3. Under **Authentication > URL Configuration**, add the redirect URL:
   ```text
   docunova://auth
   ```
4. Verify your credentials in `HomeFragment.kt` and `SettingFragment.kt`:
   ```kotlin
